"""Black box functionality tests for TimerService."""
...
async def test_basic_timer_functionality():
    ...
    assert 0.15 < elapsed < 0.3, f"Timer executed after {elapsed:.3f}s, expected ~0.2s"

    await service.stop()
    print("✓ Basic timer functionality works correctly!")


async def test_multiple_concurrent_timers():
    """Test multiple timers running simultaneously."""
    print("Testing multiple concurrent timers...")
    ...
    await service.stop()
    print("✓ Multiple concurrent timers work correctly!")


async def test_timer_cancellation():
    """Test cancelling a timer before it executes."""
    print("Testing timer cancellation...")
...
async def test_timer_reset():
    ...
    await service.stop()
    print("✓ Timer reset works correctly!")


async def test_duplicate_timer_ids():
    """Test that starting a timer with the same ID replaces the existing one."""
    print("Testing duplicate timer IDs...")
    ...
    await service.stop()
    print("✓ Duplicate timer ID handling works correctly!")
...
async def run_all_tests():
    ...
    print("=" * 50)
    print("Running TimerService Black Box Tests")
    print("=" * 50)

    try:
        await test_basic_timer_functionality()
        await test_multiple_concurrent_timers()
        await test_timer_cancellation()
        await test_timer_reset()
        await test_duplicate_timer_ids()
        await test_service_lifecycle()
        await test_exception_in_callback()

        print("=" * 50)
        print("✓ All TimerService tests passed!")
        print("=" * 50)
